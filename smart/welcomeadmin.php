    ...
    while($row = mysqli_fetch_assoc($result)) {
     ...
     if ($pg == 6)

        {
            $pstID = $_GET['PstID'];
            $id = $_GET['sql'];
            $sql = "UPDATE stories_tble SET xApproval='$id' WHERE id='$pstID'";
            $result = mysqli_query($conn, $sql); 

            echo " <script language='JavaScript'>
            location.href='welcomeadmin.php?sql=$id';
            </script>";
...
.
    ...
    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><a href= "viewstories.php?PstID=<?php echo $row["id"]; ?>&&pg=6&&sql=<?php echo $row["userID"]; ?>">View Stories</a></td>
                                            <td>
                                                <?php
                                                if($row["xApproval"] == "0")
                                                    {
                                                ?>
                                                    <a href= "welcomeadmin.php?PstID=<?php echo $row["id"]; ?>&&pg=6&&sql=1">Approve  </a>
                                                <?php
                                                    }
                                                else
                                                    {
                                            ?>
                                                <a href= "welcomeadmin.php?PstID=<?php echo $row["id"]; ?>&&pg=6&&sql=0">Dis-Approve</a>
                                                <?php
                                                    }
                                                ?>
